...
function seed(){
  ...
  if(!localStorage.getItem(TASKS_KEY)){
    ...
    const statuses:['TODO','IN_PROGRESS','DONE'][0]
    void statuses
    const tasks:DemoTask[]=titles.map((title,i)=>({
      id:i+1,title,description:['Align scope, owners and milestones.','Simplify the current experience and remove friction.','Review open feedback and close the highest-impact items.'][i%3],
...
export async function demoApi<T=unknown>(path:string,options:RequestInit={}):Promise<T>{
  ...
  const method=(options.method||'GET').toUpperCase(); const body=options.body?JSON.parse(String(options.body)):{ }
  ...
  if(path==='/token/' && method==='POST'){
    ...
    localStorage.setItem(ME_KEY,String(body.username)); return {access:'demo-access-token',refresh:'demo-refresh-token'} as T
  }
  const me=current(); if(!me) throw new Error('Session expired. Please sign in again.')
  ...
  if(path==='/tasks/' && method==='GET'){
    const all=tasks(); const visible=me.role==='ADMIN'?all:me.role==='MANAGER'?all.filter(t=>t.assigned_to===me.id||users().find(u=>u.id===t.assigned_to)?.manager===me.id):all.filter(t=>t.assigned_to===me.id)
    return visible as T
  }
  if(path==='/tasks/' && method==='POST'){
    if(me.role==='USER') throw new Error('Only Admin or Manager can create tasks.')
    const target=users().find(u=>u.id===Number(body.assigned_to)); if(!target) throw new Error('Assignee not found.')
    if(me.role==='MANAGER' && target.manager!==me.id && target.id!==me.id) throw new Error('You can only assign tasks to your team members.')
    const stamp=now(); const list=tasks(); const newTask:DemoTask={id:Math.max(0,...list.map(t=>t.id))+1,title:String(body.title||'Untitled task'),description:String(body.description||''),assigned_to:target.id,assigned_to_name:target.name||target.username,status:body.status||'TODO',created_at:stamp,updated_at:stamp}; saveTasks([newTask,...list]); return newTask as T
  }

  const taskMatch=path.match(/^\/tasks\/(\d+)\/$/)
  if(taskMatch){
    const id=Number(taskMatch[1]); const list=tasks(); const idx=list.findIndex(t=>t.id===id); if(idx<0) throw new Error('Task not found.')
    const task=list[idx]; const allowed=me.role==='ADMIN'||task.assigned_to===me.id||(me.role==='MANAGER'&&users().find(u=>u.id===task.assigned_to)?.manager===me.id); if(!allowed) throw new Error('You do not have access to this task.')
    if(method==='PATCH'){
      ...
      list[idx]={...task,...body,assigned_to_name:task.assigned_to_name,updated_at:now()}; saveTasks(list); return list[idx] as T
    }
    if(method==='DELETE'){if(me.role==='USER') throw new Error('You cannot delete tasks.'); saveTasks(list.filter(t=>t.id!==id)); return null as T}
    return task as T
  }
